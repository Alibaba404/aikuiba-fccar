...
public class LoginRoleController{
    ...
    @PostMapping
    public JSONResult save(@RequestBody @Valid LoginRole loginRole){
        return JSONResult.success(loginRoleService.save(loginRole));
    }

    @Operation( summary= "修改LoginRole",description = "基础对象修改接口")
    @Parameter(name = "loginRole",description = "修改的对象",required = true)
    @PutMapping
    public JSONResult update(@RequestBody  @Valid LoginRole loginRole){
        return JSONResult.success(loginRoleService.updateById(loginRole));
    }
    ...
    public JSONResult delete(@PathVariable("id") Long id){
        return JSONResult.success(loginRoleService.removeById(id));
    }
    ...
    public JSONResult list(@RequestBody PageQueryWrapper<LoginRole> query){
        QueryWrapper<LoginRole> wrapper = new QueryWrapper<>();
        //实体类作为查询条件
        wrapper.setEntity(query.getQuery());
        return JSONResult.success(loginRoleService.list(wrapper));
    }
    ...
    public JSONResult page(@RequestBody PageQueryWrapper<LoginRole> query){
        //分页查询
        ...
        //返回结果
        return JSONResult.success(new PageResult<LoginRole>(page.getTotal(),page.getRecords()));
    }
