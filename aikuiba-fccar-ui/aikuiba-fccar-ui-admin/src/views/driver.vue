<template>
	<div v-if="isAuth(['ROOT', 'DRIVER:SELECT'])">
		<el-form :inline="true" :model="searchForm" :rules="dataRule" ref="searchForm">
			<el-form-item prop="name"><el-input v-model="searchForm.name" placeholder="姓名" size="medium" class="input"
					clearable="clearable" /></el-form-item>
			<el-form-item>
			...
			<el-form-item prop="phone"><el-input v-model="searchForm.phone" placeholder="电话" size="medium" class="input"
					clearable="clearable" /></el-form-item>
			<el-form-item prop="idNumber"><el-input v-model="searchForm.idNumber" placeholder="身份证" size="medium"
					clearable="clearable" /></el-form-item>
			<el-form-item>
				<el-select v-model="searchForm.realAuthStatus" class="input" placeholder="实名认证" size="medium"
					clearable="clearable">
		...
		<el-table :data="dataList" border v-loading="dataListLoading" cell-style="padding: 2px 0" style="width: 100%;"
					...
					<div class="content-container">
						<div class="left">
							...
							<ul class="credentials-list">
								<li class="item">
									<el-image style="width: 350px; height: 220px" :src="scope.row.idcardFront"
										:preview-src-list="scope.row.idcardFrontList" fit="cover" />
									<p>【 身份证正面 】</p>
								</li>
								<li class="item">
									<el-image style="width: 350px; height: 220px" :src="scope.row.idcardBack"
										:preview-src-list="scope.row.idcardBackList" fit="cover" />
									<p>【 身份证背面 】</p>
								</li>
								<li class="item">
									<el-image style="width: 350px; height: 220px" :src="scope.row.idcardHolding"
										:preview-src-list="scope.row.idcardHoldingList" fit="cover" />
									<p>【 手持身份证 】</p>
								</li>
								<li class="item">
									<el-image style="width: 350px; height: 220px" :src="scope.row.drcardFront"
										:preview-src-list="scope.row.drcardFrontList" fit="cover" />
									<p>【 驾驶证正面 】</p>
								</li>
								<li class="item">
									<el-image style="width: 350px; height: 220px" :src="scope.row.drcardBack"
										:preview-src-list="scope.row.drcardBackList" fit="cover" />
									<p>【 驾驶证背面 】</p>
								</li>
								<li class="item">
									<el-image style="width: 350px; height: 220px" :src="scope.row.drcardHolding"
										:preview-src-list="scope.row.drcardHoldingList" fit="cover" />
									<p>【 手持驾驶证 】</p>
								</li>
							</ul>
						</div>
			...
			<el-table-column header-align="center" align="center" width="250" label="操作">
				<template #default="scope">
					...
					<el-button type="text" size="medium" v-if="isAuth(['ROOT', 'USER:UPDATE'])"
						@click="showRepealModel(scope.row.id)" :disabled="scope.row.realAuthStatus != 2">
						撤销认证
					</el-button>
		...
		<el-pagination @size-change="sizeChangeHandle" @current-change="currentChangeHandle" :current-page="page"
			:page-sizes="[10, 20, 50]" :page-size="rows" :total="totalCount"
